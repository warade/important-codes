...
*/
// code for one codeforces divided by zero contest #399 divide and conquire..
lld calc(lld n)
{
	lld temp=1;
	while(temp<=n){
...
lld query(lld pos,lld target, lld num)
{
	pos/=2;
	if(pos+1==target){
		return num%2;
	...
	return query(pos,target,num);
}
int main()
{
	FIO
	lld num,l,r,pos,ans=0;
	cin>>num>>l>>r;
	pos=calc(num);
	rep(i,l,r+1){
		ans+=query(pos,i,num);
