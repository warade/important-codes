...
int main()
{
	...
	rep(i,0,m) {
		queries[i].s = lower_bound(all(V), queries[i].s) - V.begin() + 1;
	}
	null->left = null->right = null;
	root[0] = null;
	rep(i,1,n+1){
		root[i] = root[i-1]-> insert(1, n+m, a[i]);
	}
	rep(i,0,m) {
		u=queries[i].f.f,v=queries[i].f.s,k=queries[i].s;
		//int idx = upper_bound(all(V), k) - V.begin() + 1;
		lld x = query(root[u-1], 1, n+m, k+1, n+m);
		lld y = query(root[v], 1, n+m, k+1, n+m);
		printf("%lld\n",y-x);	
	}
}
