...
lld q;
map<lld, lld> cnt;
void insert(lld node, lld value, lld depth){
	if(depth < 0) return;
	lld bit = (value & 1<<depth) >> depth;
	cnt[2*node + bit]++;
	insert(2*node + bit, value, depth - 1);
}
void erase(lld node, lld value, lld depth){
	if(depth < 0) return;
	lld bit = (value & 1<<depth) >> depth;
	cnt[2*node + bit]--;
	erase(2*node + bit, value, depth - 1);
}
lld query(lld node, lld value, lld depth){
	if(depth < 0) return 0;
	lld bit = (value & 1<<depth) >> depth;
	lld opp = !bit;
	if(cnt[2*node + opp] > 0) return (1<<depth) + query(2*node + opp, value, depth  - 1);
	else return query(2*node + bit, value, depth - 1);
}
int main(){
	FIO
	cin >> q;
